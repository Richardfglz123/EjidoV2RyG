...
@section('content')
    ...
    <div class="card card-ejidal shadow-sm">
            ...
            <table class="table table-hover table-striped align-middle mb-0">
                ...
                @forelse($data as $fila)
                    ...
                    <tr>
                        ...
                        <td class="ps-3 text-center fw-bold text-muted">
                            {{ $fila->Num_Ejidatario }}
                        </td>
                        ...
                        <td>
                            ...
                            <div class="d-md-none mt-2">
                                ...
                                <div class="small text-muted mb-1">
                                    <strong>CURP:</strong> {{ $fila->CURP ?? 'N/A' }}
                                </div>

                                <div class="small text-muted mb-2">
                                    <strong>RFC:</strong> {{ $fila->RFC ?? 'N/A' }}
                                </div>
                                ...
                                <div class="d-flex align-items-center gap-2 mt-1">
                                    ...
                                    @if(!empty($fila->qr_payload))
                                        <button
                                                type="button"
                                                class="btn btn-xs btn-outline-dark py-0 px-2 small"
                                                style="font-size: 0.75rem;"
                                                data-bs-toggle="modal"
                                                data-bs-target="#modalQR{{ $fila->Id_Ejidatario }}">
        ...
        <div class="card-footer bg-light border-top d-flex flex-column flex-sm-row justify-content-between align-items-center gap-2">
            ...
            <div>
                <a href="{{ route('reportes.ejidatarios.pdf') }}"
                ...
                <a href="{{ route('reportes.ejidatarios.excel') }}"
    ...
    @foreach($data as $fila)
        ...
        @if(!empty($fila->qr_payload))
            ...
            @php
                ...
                $nombreCompletoModal = preg_replace(
                    '/\s+/',
                    ' ',
                    trim(str_ireplace(
                        ['\n', "\n", "\r"],
                        ' ',
                        implode(' ', array_filter($nombrePartes))
                    ))
                );
            @endphp

            <div class="modal fade"
                 id="modalQR{{ $fila->Id_Ejidatario }}"
                 tabindex="-1"
                 aria-hidden="true">
